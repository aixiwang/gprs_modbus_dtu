...
//-------------------------------------------------------------
// func 03 steps:
//     1. clear uart rx buff
//     2. create_03_pkg
//     3. send the pkg
//     4. read_03_response_pkg
//     5. parse the data
//
//-------------------------------------------------------------
// create_03_pkg
...
int create_03_pkg(u8* buff,u8 addr,u16 start,u16 lens,u8* p_pkg_len)
{
	...
	// TODO, customer up limit num for your app
	if ((lens * 2 +  7) > 64)
		return -1;
...
int read_03_response_pkg(u8* buff,u8 addr, u8 n)
{
	...
	{
		lcd_putstr("0700");
		if (get_uart1_rx_buff_len() >= cnt)
		{
			EA = 0;
			for(i=0; i<cnt; i++)
			{
				buff[i] = get_uart1_buff(i);
			}
			EA = 1;
